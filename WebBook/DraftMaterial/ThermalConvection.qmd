...
from equation (@eq-biharm)). Therefore the energy conservation equation
becomes
...
![Critical Rayleigh Number determination. A plot of growth rates for harmonic perturbations as a function of wavenumber for different ${\rm Ra}$. The critical value occurs when the maximum of the curve just touches the horizontal axis at zero.](../Images/Diagrams/crit_ra.png){#fig-critical-ra width=450px}
...
![Boundary Layer Theory in its simplest form](../Images/Diagrams/blt.png){#fig-boundary-layer-theory-1 width=450px}
...
![Boundary Layer Theory which accounts for the thickness variations along the boundary layer as it matures.](../Images/Diagrams/blt2.png){#fig-boundary-layer-theory-2 width=450px}
...
![Cooling half-space calculation](../Images/Diagrams/CoolingHalfSpaceSolutions1.png){#fig-cooling-half-space width=450px}
...
**Changes made:**
- Created 6 exercises aligned with thermal convection concepts
- Added 2 worked solutions with detailed explanations
- Exercises cover: critical Rayleigh number, Nusselt number, boundary layers, time-dependence, internal heating, and compressibility
- Need to add: exercises on linear stability theory, perturbation analysis, experimental validation
:::
...
a) Explain physically why there is a critical Rayleigh number below which convection cannot occur

b) How does the critical Rayleigh number depend on the boundary conditions (free-slip vs. no-slip)?

c) Why is the critical Rayleigh number different for different wavelengths of perturbations?

### Exercise 2: Nusselt Number Scaling

a) Derive the definition of the Nusselt number as the ratio of total heat flux to conductive heat flux

b) Explain why $Nu = 1$ for pure conduction

c) For mantle convection with $Ra \sim 10^7$, estimate the Nusselt number using the scaling $Nu \sim Ra^\beta$. What value of $\beta$ is appropriate?

### Exercise 3: Thermal Boundary Layer Thickness ⭐

Using boundary layer theory:

a) Estimate the thermal boundary layer thickness $\delta$ as a function of Rayleigh number

b) For Earth's mantle with $Ra \sim 10^7$ and layer depth $h = 2900$ km, calculate $\delta$

c) Compare this to estimates from seismic observations of the D" layer

### Exercise 4: Steady vs. Time-Dependent Convection

a) At what Rayleigh number does the transition from steady to time-dependent convection typically occur?

b) Describe the physical mechanism that causes time-dependence at high $Ra$

c) How does this relate to plate tectonics on Earth?

### Exercise 5: Internal Heating

Modify the governing equations to include internal heating rate $H$ (W/kg):

a) Add the internal heating term to the energy equation

b) Define an appropriate internal heating Rayleigh number

c) Discuss how internal heating changes the convection pattern compared to bottom heating only

### Exercise 6: Compressibility Effects

a) Under what conditions can the Boussinesq approximation be used for planetary mantles?

b) Derive the anelastic approximation starting from the full compressible equations

c) Calculate the adiabatic temperature gradient for Earth's mantle and compare to the actual geotherm

## Solutions to Selected Exercises

:::{.callout-tip collapse="true"}
## Solution to Exercise 1: Critical Rayleigh Number
...
Below $Ra_c$: Conduction is stable (viscous damping dominates)
Above $Ra_c$: Convection occurs (buoyancy dominates)
...
:::{.callout-tip collapse="true"}
## Solution to Exercise 3: Thermal Boundary Layer
...
Seismic observations suggest the D" layer (thermal boundary layer at CMB) is $\sim$100-300 km thick. Our estimate is much thinner, suggesting:

1. The D" layer may not be purely thermal (could include chemical layering)
2. Local Rayleigh number near CMB might be lower than whole-mantle value
3. Time-dependent effects and plumes complicate the simple boundary layer picture
4. The scaling assumes steady state, but Earth's convection is time-dependent
